...
impl<'a, Tree, G> CompoundProof<'a, DrgPoRep<'a, Tree, G>, DrgPoRepCircuit<'a, Tree::Hasher>>
    for DrgPoRepCompound<Tree::Hasher, G>
where
    Tree: 'a + MerkleTreeTrait,
    G::Key: AsRef<<Tree::Hasher as Hasher>::Domain>,
    G: 'a + Graph<Tree::Hasher> + ParameterSetMetadata + Sync + Send,
{
    fn generate_public_inputs(
        pub_in: &<DrgPoRep<'a, Tree, G> as ProofScheme<'a>>::PublicInputs,
        pub_params: &<DrgPoRep<'a, Tree, G> as ProofScheme<'a>>::PublicParams,
    ...
    fn circuit(
        public_inputs: &<DrgPoRep<'a, Tree, G> as ProofScheme<'a>>::PublicInputs,
        component_private_inputs: <DrgPoRepCircuit<Tree::Hasher> as CircuitComponent>::ComponentPrivateInputs,
        proof: &<DrgPoRep<'a, Tree, G> as ProofScheme<'a>>::Proof,
        public_params: &<DrgPoRep<'a, Tree, G> as ProofScheme<'a>>::PublicParams,
        _partition_k: Option<usize>,
    ) -> Result<DrgPoRepCircuit<'a, Tree::Hasher>> {
...
mod tests {
    ...
    fn drgporep_test_compound<Tree: MerkleTreeTrait>() {
        ...
        let (tau, aux) = drg::DrgPoRep::<Tree, BucketGraph<Tree::Hasher>>::replicate(
        ...
        {
            let gparams =
                DrgPoRepCompound::<Tree::Hasher, _>::groth_params(Some(rng), &public_params.vanilla_params)
